...
public class loggerToFileTelBank {
    private static final loggerToFileTelBank inst = new loggerToFileTelBank();

    private PersianDateTime PDT = new PersianDateTime();
    strUtils strutils = new strUtils();

    private loggerToFileTelBank() {
        super();
    }
    public synchronized void logInfo(String content) {
        try {
            ...
            String Path =strutils.leftString(filePath,filePath.length()-11 )+ "Log/" + PDT.getShamsiDateForFileName() + "-Info.log";
        //    Util.printMessage(Path,false);
    ...
    public synchronized void logInfoTelBank(String content) {
        try {
           ...
           // Util.printMessage(Path, false, false);

            String filePath=FileOfSettings.toString();
            String Path =strutils.leftString(filePath,filePath.length()-11 )+ "Log/" + PDT.getShamsiDateForFileName() + "- Errors.log";
          //  Util.printMessage(Path,false);
            File file = new File(Path);
            if (!file.exists()) {
                file.createNewFile();
    ...
    public synchronized void logError(String className, String lineNumber, String content) {
        try {
            ...
            File FileOfSettings = new File(PropertiesUtils.class.getProtectionDomain().getCodeSource().getLocation().getPath());
          //  String Path = FileOfSettings.toString() + "\\Log\\" + PDT.getShamsiDateForFileName() + "- Errors.log";
    ...
    public synchronized void logPureText(String content) {
        try {

            File file = new File(Util.ivrPath+"Fax/faxlog.html");
    ...
    public static loggerToFileTelBank getInstance() {
        return inst;
    }
