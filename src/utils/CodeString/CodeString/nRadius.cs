...
namespace System.Net.nRadius
{
    public class nRadius_Client
    {
        ...
        private int ProcessServerResponse(byte[] receivedBytes)
        {
            ...
            // 1. Case: No paket data (no received data) --> Communication error
            if (pPaketLen == 0) { pReturn = -1; }
            else // there is paket data
            {   // ##########################################################################################
                // Checking the Radius Code (first Bit) and the minimum paket length of 20 bytes
                if (pPaketLen >= 20) 
                { 
                ...
                else { pReturn = -3; pErrorOccured = true; }
                ...
                if (pPaketLen>=20 && !pErrorOccured)
                {
                    ...
                    if (pIdentifier != pClientIdentifier) { pReturn = -5; pErrorOccured = true; }
                }
                else { pReturn = -3; pErrorOccured = true; }
                ...
                if (pPaketLen >= 20 && !pErrorOccured)
                {
                    ...
                    DebugOutput("Paket Length (Radius-Response): " + pLen.ToString());
                    if (pLen != pPaketLen) { pReturn = -6; pErrorOccured = true; }
                }
                else { pReturn = -3; pErrorOccured = true; }
                ...
                if (pPaketLen >= 20 && !pErrorOccured)
                {
                    ...
                    // Getting the Response Authenticator from the received Paket
        ...
        private void DebugOutput(string Output)
        {
            if (pDebug)
            {
                ...
                System.IO.StreamWriter myDebugWriter = new System.IO.StreamWriter("nRadius_Debug.txt",true);

                myDebugWriter.WriteLine("[DEBUG] " + Output);
