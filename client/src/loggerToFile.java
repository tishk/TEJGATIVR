...
public class loggerToFile {
    ...
    public synchronized void logInfo(String content) {
        ...
        File file =null;
        try {
            final File FileOfSettings=new File(loggerToFile.class.getProtectionDomain().getCodeSource().getLocation().getPath());
            String Path1 =FileOfSettings.toString();
            int i="client.jar".length();
            try{
                 file = new File(Path1+  "\\" + PDT.getShamsiDateForFileName() + "-Info.log");
                if (!file.exists()) {
                    file.createNewFile();
                }
            }catch (Exception e){
                try{
                    String Path =Path1.substring(0,Path1.length()-i);
                     file = new File(Path+  "\\" + PDT.getShamsiDateForFileName() + "-Info.log");
                    if (!file.exists()) {
                        file.createNewFile();
                    }
                }catch (Exception ee){
            ...
            logPureStringForInfo(file, content);
            file=null;
        } catch (Exception e) {
            e.printStackTrace();
        }
    ...
    private void logPureStringForInfo(File file, String content) {
        try {
            FileWriter fw = new FileWriter(file, true); //the true will append the new data
            fw.write(content + " \n");
            fw.close();
